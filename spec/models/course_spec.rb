...
RSpec.describe Course, type: :model do
  ...
  describe "ResponseHandler.request_and_save" do
    ...
    context "200 status, valid body" do
      ...
      it "saves data into DB" do
        expect { ResponseHandler.request_and_save(resp) }.to change(Course, :count).by(1)
      end

      it "doesn't duplicate data" do
        ResponseHandler.request_and_save(resp)
        expect { ResponseHandler.request_and_save(resp) }.not_to change(Course, :count)
      end
      ...
      context "existed course with 404 response_status" do
        ...
        it "changes status to 200" do
          expect {
            ResponseHandler.request_and_save(resp) && course.reload
      ...
      context "pirvate course" do
        ...
        it "doesn't save private course" do
          expect {
            ResponseHandler.request_and_save(test.get '/privy')
